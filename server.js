...
const VERSION = "1.7-baked-potato-dressing-drizzle-prompt-backlog";
...
const SIDE_UPGRADES = { "buffalo ranch fries": 1.5 };

const MENU = {
  wings_standalone: {
    label: "Classic Wings",
    family: "wings",
    pricesByQuantity: { 6: 10.1, 9: 14.2, 12: 18.3, 18: 23.65, 24: 30.65, 48: 58.5 },
  ...
  classic_burger: { label: "Classic Burger", family: "burger", price: 8.85, requiredSlots: [] },
  buffalo_burger: { label: "Buffalo Burger", family: "burger", price: 9.45, requiredSlots: [] },
  chicken_sandwich: { label: "Chicken Sandwich", family: "sandwich", price: 8.85, requiredSlots: ["chickenStyle"] },
  flyin_burger: { label: "Flyin’ Burger", family: "burger", price: 11.55, requiredSlots: ["chickenStyle"] },
  ...
  combo_baked_potato: {
    ...
    sauceLimit: 1,
    requiredSlots: ["protein", "sauces", "drizzle", "drinkType"],
    drinkIncluded: "24oz soft drink"
  },
  house_salad: { label: "House Salad", family: "salad", price: 7.7, requiredSlots: ["dressing"] },
  flyin_salad: { label: "Flyin’ Salad", family: "salad", price: 11.3, requiredSlots: ["chickenStyle", "dressing"] },
  flyin_fries: { label: "Flyin’ Fries", family: "loaded_fries", price: 9.85, requiredSlots: [] },
  pork_belly_fries: { label: "Pork Belly Fries", family: "loaded_fries", price: 12.25, requiredSlots: [] },
  chicken_parmesan_fries: { label: "Chicken Parmesan Fries", family: "loaded_fries", price: 12.25, requiredSlots: [] },
  ...
  flyin_corn: { label: "Flyin’ Corn", family: "side", price: 5.9, requiredSlots: [] },
  ...
  kids_cheeseburger: { label: "Kids Cheeseburger", family: "kids", price: 9.25, requiredSlots: [] },
  soft_drink_24oz: { label: "24oz Soft Drink", family: "drink", price: 2.99, requiredSlots: [] },
  bottled_water: { label: "Bottled Water", family: "drink", price: 2, requiredSlots: [] }
};
...
function normalizeDressing(value = "") {
  ...
  if (!val) return "";
  if (["no", "none", "nothing", "no dressing", "no drizzle", "without dressing", "without drizzle"].includes(val)) {
    return "no drizzle";
  }
...
function normalizeSaucePlacement(value = "", itemId = "") {
  const placement = phraseKey(value);
  if (placement.includes("side")) return "on the side";
  if (placement.includes("on top") || placement.includes("encima")) return "on top";
  if (itemId === "combo_baked_potato") return "on top";
  return placement || "";
}
...
function normalizeItemId(value = "") {
  ...
  const underscoreCandidate = phraseKey(raw).replace(/\s+/g, "_");
  if (MENU[underscoreCandidate]) return underscoreCandidate;
...
function normalizeItemIdFromText(text = "") {
  ...
  if (MENU[t.replace(/\s+/g, "_")]) return t.replace(/\s+/g, "_");
  ...
  if (t.includes("half rack") && (t.includes("4 bone") || t.includes("four bone") || t.includes("4 wing"))) return "combo_half_rack_4_bonein";
  if (t.includes("half rack combo") || t.includes("medio rack combo")) return "combo_half_rack";
  if (t.includes("fish fry combo") || t.includes("4 piece fish") || t.includes("4 fish combo") || t.includes("fish combo")) return "combo_fish";
  ...
  if (t.includes("classic wings") || t.includes("bone in") || t.includes("traditional wings") || t.includes("wings") || t.includes("alitas")) return "wings_standalone";
  ...
  if (t.includes("flyin salad") || t.includes("flying salad")) return "flyin_salad";
  if (t.includes("house salad")) return "house_salad";
  if (t.includes("chicken parmesan fries")) return "chicken_parmesan_fries";
  if (t.includes("pork belly fries")) return "pork_belly_fries";
  if (t.includes("flyin fries") || t.includes("flying fries")) return "flyin_fries";
...
function normalizePayload(raw = {}) {
  const combinedText = [raw.text, raw.item, raw.itemId, raw.type, raw.name, raw.description].filter(Boolean).join(" ");
  const itemId = normalizeItemId(raw.itemId || "") || normalizeItemId(raw.item || "") || normalizeItemIdFromText(combinedText);
  ...
  if (itemId === "combo_baked_potato") {
    if (!drizzle && dressing) drizzle = dressing;
    if (!dressing && drizzle) dressing = drizzle;
    if (!drizzle && !dressing && dips.length > 0) {
      drizzle = dips[0];
      dressing = dips[0];
    }
  }
  ...
  const quantity = def.pricesByQuantity ? Number(raw.quantity || detectQuantity(combinedText) || 0) : 0;
  ...
  return {
    ...
    dressing,
    drizzle,
    saucePlacement: normalizeSaucePlacement(raw.saucePlacement, itemId),
    ...
    modifications: asArray(raw.modifications).map(String).filter(Boolean)
  };
}
...
function fail(code, message, details = []) {
  return { success: false, ok: false, speak: cleanSpeak(message), error: { code, message: cleanSpeak(message), details } };
}
...
function missingSlotMessage(slot, lang) {
  const messages = {
    ...
    sideChoice: speak(lang, "What side would you like: regular fries, sweet potato fries, potato salad, or buffalo ranch fries?", "¿Qué acompañamiento quieres: papas regulares, papas de camote, ensalada de papa o buffalo ranch fries?"),
    ...
    drinkType: speak(lang, "What drink would you like with that?", "¿Qué bebida quieres con eso?"),
    dressing: speak(lang, "What dressing or drizzle would you like?", "¿Qué aderezo quieres encima?"),
    drizzle: speak(lang, "What dressing or drizzle would you like?", "¿Qué aderezo quieres encima?"),
...
function slotMissing(item, slot) {
  if (slot === "quantity") return !item.quantity;
  if (slot === "sauces") return item.sauces.length === 0 && item.saucePlacement !== "no sauce";
  if (slot === "dips") return item.dips.length === 0;
  if (slot === "sideChoice") return !item.sideChoice;
  if (slot === "protein") return !item.protein;
  if (slot === "chickenStyle") return !item.chickenStyle;
  if (slot === "drinkType") return !item.drinkType;
  if (slot === "dressing") return !item.dressing;
  if (slot === "drizzle") return !item.drizzle;
  if (slot === "cornRibsSauce") return !item.cornRibsSauce;
  if (slot === "wingSauce") return !item.wingSauce;
  if (slot === "wingDip") return !item.wingDip;
  return false;
}
...
function validateRequiredSlots(item, def) {
  ...
  for (const slot of def.requiredSlots || []) {
    if (slotMissing(item, slot)) missing.push(slot);
  }
  if (item.itemId === "combo_baked_potato" && item.protein === "chicken" && !item.chickenStyle) missing.push("chickenStyle");
...
function validateItem(raw = {}) {
  ...
  if (!def) {
    return fail(
      "INVALID_ITEM",
      speak(lang, "I am sorry, I do not see that item on our menu. What would you like to order?", "Lo siento, no veo ese producto en el menú. ¿Qué te gustaría ordenar?"),
      [raw.itemId || raw.item || raw.text || ""]
    );
  }

  if (item.sauces.includes("lemon pepper")) {
    return fail("CORRECTION_REQUIRED", speak(lang, "We have that as lime pepper. Is that okay?", "Aquí la tenemos como lime pepper, ¿está bien?"), ["lemon pepper"]);
  }

  const missing = validateRequiredSlots(item, def);
  if (missing.length) return fail("MISSING_SLOT", missingSlotMessage(missing[0], lang), missing);

  for (const sauce of item.sauces) {
    if (!SAUCES.includes(sauce)) {
      return fail("INVALID_SAUCE", speak(lang, `I do not have ${sauce} as a sauce. What sauce would you like?`, `No tengo ${sauce} como salsa. ¿Qué salsa quieres?`), [sauce]);
    }
  }

  for (const dip of item.dips) {
    if (!DIPS.includes(dip)) {
      return fail("INVALID_DIP", speak(lang, `I do not have ${dip} as a dip. What dip would you like?`, `No tengo ${dip} como aderezo. ¿Qué aderezo quieres?`), [dip]);
    }
  }

  for (const dipLike of [item.dressing, item.drizzle, item.wingDip].filter(Boolean)) {
    if (!DIPS.includes(dipLike) && !NO_DRESSING_VALUES.includes(dipLike)) {
      return fail("INVALID_DRESSING", speak(lang, `I do not have ${dipLike} as a dressing. What dressing would you like?`, `No tengo ${dipLike} como aderezo. ¿Qué aderezo quieres?`), [dipLike]);
    }
  }

  if (item.wingSauce && !SAUCES.includes(item.wingSauce)) {
    return fail("INVALID_WING_SAUCE", speak(lang, `I do not have ${item.wingSauce} as a wing sauce. What sauce would you like for the wings?`, `No tengo ${item.wingSauce} como salsa para alitas. ¿Qué salsa quieres?`), [item.wingSauce]);
  }

  if (item.cornRibsSauce && !SAUCES.includes(item.cornRibsSauce)) {
    return fail("INVALID_CORN_RIBS_SAUCE", speak(lang, `I do not have ${item.cornRibsSauce} as a sauce. What sauce would you like for the corn ribs?`, `No tengo ${item.cornRibsSauce} como salsa. ¿Qué salsa quieres para los corn ribs?`), [item.cornRibsSauce]);
  }

  if (def.pricesByQuantity && !def.pricesByQuantity[item.quantity]) {
    return fail("INVALID_QUANTITY", speak(lang, "Available quantities are 6, 9, 12, 18, 24, or 48.", "Las cantidades disponibles son 6, 9, 12, 18, 24 o 48."), [item.quantity]);
  }

  if (def.pricesByProtein && !def.pricesByProtein[item.protein]) {
    return fail("INVALID_PROTEIN", speak(lang, "Protein choices are chicken, steak, pork belly, or no protein.", "Las proteínas son pollo, carne asada, pork belly o sin proteína."), [item.protein]);
  }

  if (def.sauceLimitByQuantity) {
    const limit = def.sauceLimitByQuantity[item.quantity];
    if (item.sauces.length > limit) return fail("TOO_MANY_SAUCES", speak(lang, `That order includes up to ${limit} sauce${limit > 1 ? "s" : ""}.`, `Esa orden incluye hasta ${limit} salsa${limit > 1 ? "s" : ""}.`), item.sauces);
  }

  if (def.dipLimitByQuantity) {
    const limit = def.dipLimitByQuantity[item.quantity];
    if (item.dips.length > limit) return fail("TOO_MANY_DIPS", speak(lang, `That order includes up to ${limit} dip${limit > 1 ? "s" : ""}.`, `Esa orden incluye hasta ${limit} aderezo${limit > 1 ? "s" : ""}.`), item.dips);
  }

  if (def.sauceLimit !== undefined && item.sauces.length > def.sauceLimit) {
    return fail("TOO_MANY_SAUCES", speak(lang, `That item includes up to ${def.sauceLimit} sauce.`, `Ese producto incluye hasta ${def.sauceLimit} salsa.`), item.sauces);
  }

  if (def.dipLimit !== undefined && item.dips.length > def.dipLimit) {
    return fail("TOO_MANY_DIPS", speak(lang, `That item includes up to ${def.dipLimit} dip.`, `Ese producto incluye hasta ${def.dipLimit} aderezo.`), item.dips);
  }

  if (item.sideChoice && !SIDE_CHOICES.includes(item.sideChoice)) {
    return fail("INVALID_SIDE", speak(lang, "Side choices are regular fries, sweet potato fries, potato salad, or buffalo ranch fries.", "Los acompañamientos son papas regulares, papas de camote, ensalada de papa o buffalo ranch fries."), [item.sideChoice]);
  }

  const extraCharges = [];
  if (Array.isArray(def.extraCharges)) extraCharges.push(...def.extraCharges);
  if (item.sideChoice && SIDE_UPGRADES[item.sideChoice]) {
    extraCharges.push({ label: "Buffalo Ranch Fries combo upgrade", amount: SIDE_UPGRADES[item.sideChoice] });
  }
...
function cartTotals(items = []) {
  ...
  return { subtotal, tax, total, taxRate: TAX_RATE, requiresPaymentBeforePreparation: total > 50 };
}

function itemSummary(item, lang = "en") {
  const parts = [];
  if (item.quantity && (item.family === "wings" || item.family === "boneless")) parts.push(`${item.quantity} ${item.label}`);
  ...
  if (item.wingSauce) parts.push(`${lang === "es" ? "salsa de alitas" : "wing sauce"} ${item.wingSauce}`);
  if (item.wingDip) parts.push(`${lang === "es" ? "aderezo de alitas" : "wing dip"} ${item.wingDip}`);
  ...
  if (item.protein) parts.push(`${lang === "es" ? "proteína" : "protein"} ${item.protein}`);
  if (item.drizzle && !NO_DRESSING_VALUES.includes(item.drizzle)) parts.push(`${lang === "es" ? "aderezo" : "dressing"} ${item.drizzle}`);
  if (item.drinkType) parts.push(`${lang === "es" ? "bebida" : "drink"} ${item.drinkType}`);
...
function cartSummary(cart, lang = "en") {
  ...
  return speak(lang, `Your order: ${lines.join("; ")}.`, `Tu orden: ${lines.join("; ")}.`);
}
...
function updateCustomerMemory(phone, name, order) {
  if (!phone) return;
  const current = customers[phone] || { phone, name: name || "", visits: 0, lastOrder: null, orders: [] };
...
async function submitToPOS(order) {
  if (POS_MODE === "stub") {
    ...
    return { success: true, mode: "stub", posOrderId: order.orderId, message: "Order prepared for POS submission." };
  }
  return { success: false, mode: POS_MODE, message: "SpotOn integration is not configured yet." };
}
...
function detectTransferIntent(text = "") {
  ...
  return phrases.some((phrase) => t.includes(phraseKey(phrase)));
}
...
function transferResponse(lang) {
  return {
    ...
    speak: speak(lang, "I can connect you with the restaurant so someone can help you.", "Te puedo transferir con el restaurante para que alguien te ayude.")
  };
}
...
async function handleAction(payload = {}) {
  ...
  if (payload.text && detectTransferIntent(payload.text)) return transferResponse(lang);
  ...
  if (action === "get_customer") {
    ...
    return {
      ...
      ok: true,
      customer,
      speak: customer ? speak(lang, `Welcome back ${customer.name || ""}.`, `Bienvenido de nuevo ${customer.name || ""}.`) : speak(lang, "First time customer.", "Cliente nuevo.")
    };
  }

  if (action === "set_customer_name") {
    cart.customerName = payload.customerName || payload.name || cart.customerName;
    return { success: true, ok: true, speak: speak(lang, "Thank you.", "Gracias.") };
  }

  if (action === "clear_cart") {
    cart.items = [];
    return { success: true, ok: true, cart, totals: cartTotals(cart.items), speak: speak(lang, "The order has been cleared.", "La orden quedó borrada.") };
  }

  if (action === "get_cart") {
    return { success: true, ok: true, cart, totals: cartTotals(cart.items), speak: cartSummary(cart, lang) };
  }
  ...
  if (action === "transfer_message") {
    const message = {
      ...
      callbackPhone: payload.callbackPhone || payload.phone || "",
      reason: payload.reason || payload.text || "",
      language: lang,
      createdAt: new Date().toISOString()
    };
    transferMessages.push(message);
    return { success: true, ok: true, transferMessage: message, speak: speak(lang, "I saved that information and will connect you now.", "Guardé esa información y ahora te voy a conectar.") };
  }

  if (action === "finalize_order") {
    if (!cart.items.length) {
      return fail("EMPTY_CART", speak(lang, "I do not have any items in the order yet. What would you like to order?", "Todavía no tengo productos en la orden. ¿Qué te gustaría ordenar?"), []);
    }
    ...
    const paymentLine = totals.requiresPaymentBeforePreparation
      ? speak(lang, " Because this order is over fifty dollars, payment is required before preparation.", " Como esta orden es de más de cincuenta dólares, requiere pago antes de prepararla.")
      : "";
  ...
  const totals = cartTotals(cart.items);
  return { success: true, ok: true, item: validation.item, cart, totals, speak: speak(lang, "Perfect, I added it.", "Perfecto, lo agregué.") };
}
...
function getToolCallId(toolCall = {}) {
  return toolCall.id || toolCall.toolCallId || toolCall.function?.id || toolCall.functionCallId || "";
}
...
app.get("/", (req, res) => {
  res.json({ ok: true, service: "Flaps & Racks AI Cashier Backend", version: VERSION });
});

app.get("/health", (req, res) => {
  res.json({ ok: true, version: VERSION, taxRate: TAX_RATE, restaurantPhone: RESTAURANT_PHONE, posMode: POS_MODE });
});

app.get("/transfer-messages", (req, res) => {
  res.json({ ok: true, count: transferMessages.length, messages: transferMessages.slice(-50) });
});
  ...
  try {
    ...
    if (toolCalls.length > 0) {
      ...
      for (const toolCall of toolCalls) {
        ...
        console.log("VAPI TOOL CALL RESULT:", JSON.stringify(result, null, 2));

        results.push({ toolCallId: getToolCallId(toolCall), result: cleanSpeak(result.speak || "Okay.") });
      }
    ...
    return res.json({
      ...
      speak: speakMessage,
      result: speakMessage,
      error: { code: "SERVER_ERROR", message: err.message || speakMessage }
    });
  }
});
